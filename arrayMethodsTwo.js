...
// CODE HERE
let arr2 = numbers.map((element) => element/2)
console.log(arr2)
...
// CODE HERE
let newCities = cities.filter((town) => {
if(town.endsWith('i'))
return town
})
...
fruits.forEach((element, index) => {
  ...
  }else console.log(`The fruit with index ${index} is neither.`)
})
...
// The fruit with index 0 is neither.
// The fruit with index 1 is an orange.
// The fruit with index 2 is neither.
// The fruit with index 3 is an apple.
...
// CODE HERE
let foodRice = foods.filter((rice) => rice.tags.includes("rice"))
console.log(foodRice)

// THE TOTAL
let total = foodRice.reduce((acc,item) => (acc + item.price), 0)
console.log(total)